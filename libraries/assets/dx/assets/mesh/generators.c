...
dx_result dx_asset_mesh_append_quadriliteral(dx_asset_mesh* mesh) {
  ...
  // left top
  if (dx_asset_mesh_append_vertex(mesh, &p[3], &c[3], &t[3])) {
    return DX_FAILURE;
  }

  // left bottom
  if (dx_asset_mesh_append_vertex(mesh, &p[0], &c[0], &t[0])) {
    return DX_FAILURE;
  }

  // right top
  if (dx_asset_mesh_append_vertex(mesh, &p[2], &c[2], &t[2])) {
    return DX_FAILURE;
  }
  ...
  // right bottom
  if (dx_asset_mesh_append_vertex(mesh, &p[1], &c[1], &t[1])) {
    return DX_FAILURE;
  }
...
//#define DX_ASSET_MESH_ON_OCTAHEDRON_NO_TRANSFORM
