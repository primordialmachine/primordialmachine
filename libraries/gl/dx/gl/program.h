...
DX_DECLARE_OBJECT_TYPE("dx.gl.program",
                       dx_gl_program,
                       dx_val_program)
...
static inline dx_gl_program_dispatch* DX_GL_PROGRAM_DISPATCH(void* p) {
  return (dx_gl_program_dispatch*)p;
}

struct dx_gl_program_dispatch {
  dx_val_program _parent;
};
