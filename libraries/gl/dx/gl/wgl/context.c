...
static dx_result leave_frame(dx_gl_wgl_context* self) {
  DX_GL_CONTEXT(self)->glFlush();
  SwapBuffers(self->window->dc);
  return DX_SUCCESS;
}

static dx_result get_canvas_size(dx_gl_wgl_context* SELF, dx_i32* width, dx_i32* height) {
  return dx_gl_wgl_window_get_canvas_size(SELF->window, width, height);
}
...
dx_result dx_gl_wgl_context_construct(dx_gl_wgl_context* SELF, dx_gl_wgl_window* window) {
...
dx_result dx_gl_wgl_context_create(dx_gl_wgl_context** RETURN, dx_gl_wgl_window* window) {
