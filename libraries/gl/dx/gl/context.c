...
static dx_result get_renderer(dx_string** RETURN, dx_gl_context* SELF) {
  GLubyte const* p = SELF->glGetString(GL_RENDERER);
  return dx_string_create(RETURN, p, strlen(p));
}

static dx_result get_vendor(dx_string** RETURN, dx_gl_context* SELF) {
  GLubyte const* p = SELF->glGetString(GL_VENDOR);
  return dx_string_create(RETURN, p, strlen(p));
}

static dx_result get_major_version(int32_t* RETURN, dx_gl_context* SELF) {
  GLint v;
...
static dx_result get_minor_version(int32_t* RETURN, dx_gl_context* SELF) {
  GLint v;
...
static dx_string* get_information(dx_gl_context* self) {
...
static dx_result enter_frame(dx_gl_context* SELF) {
  SELF->glDepthMask(SELF->depth_write_enabled ? GL_TRUE : GL_FALSE);
  return DX_SUCCESS;
}

static dx_result bind_texture(dx_gl_context* ctx, dx_size unit, dx_gl_texture* texture) {
  if (texture) {
    if (dx_gl_texture_upload(texture)) {
      return DX_FAILURE;
    }
...
static dx_result create_buffer(dx_gl_buffer** RETURN, dx_gl_context* SELF) {
  return dx_gl_buffer_create(RETURN, SELF);
}

static dx_result create_cbinding(dx_val_cbinding** RETURN, dx_gl_context* SELF) {
...
static dx_result create_vbinding(dx_gl_vbinding** RETURN, dx_gl_context* SELF, DX_VERTEX_FORMAT vertex_format, dx_gl_buffer* buffer) {
  return dx_gl_vbinding_create(RETURN, vertex_format, buffer);
}

static dx_result create_program(dx_gl_program** RETURN, dx_gl_context* SELF, dx_val_program_text* program_text) {
  if (!program_text || DX_VAL_PROGRAM_TEXT_TYPE_VERTEX_FRAGMENT != program_text->type) {
    dx_set_error(DX_ERROR_INVALID_ARGUMENT);
    return DX_FAILURE;
  }
  return dx_gl_program_create(RETURN, SELF, program_text->vertex_program_text->program_text, program_text->fragment_program_text->program_text);
}

static dx_result create_texture(dx_gl_texture** RETURN, dx_gl_context* SELF) {
  return dx_gl_texture_create(RETURN, SELF);
}

static dx_result execute_commands(dx_gl_context* SELF, dx_val_command_list* commands) {
...
static void dx_gl_context_dispatch_construct(dx_gl_context_dispatch* SELF) {
...
dx_result dx_gl_context_construct(dx_gl_context* SELF, void *(*link)(char const *name)) {
  dx_rti_type* TYPE = dx_gl_context_get_type();
  if (!TYPE) {
    return DX_FAILURE;
  }
