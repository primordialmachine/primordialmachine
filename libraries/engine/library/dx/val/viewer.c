...
Core_Result dx_val_viewer_get_projection_matrix(DX_MAT4* RETURN, dx_val_viewer* SELF, Core_Integer32 canvas_width, Core_Integer32 canvas_height) {
  ...
  if (result) {
    ...
    if (optics1->aspectRatio) {
      Core_Memory_deallocate(optics1->aspectRatio);
      optics1->aspectRatio = NULL;
    }
    Core_Real32 aspect_ratio = (Core_Real32)canvas_width / (Core_Real32)canvas_height;
    if (optics1->aspectRatio) {
      aspect_ratio = *optics1->aspectRatio;
    }
    dx_mat4_set_perspective(&SELF->projection_matrix, optics1->fieldOfViewY, aspect_ratio, optics1->near, optics1->far);
