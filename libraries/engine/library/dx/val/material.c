...
static Core_Result add_to_backend(dx_val_material* SELF) {
  if (SELF->material_asset->ambientTextureReference && SELF->material_asset->ambientTextureReference->object) {
    Core_Assets_Texture* asset_ambient_texture = CORE_ASSETS_TEXTURE(SELF->material_asset->ambientTextureReference->object);
...
Core_Result dx_val_material_construct(dx_val_material* SELF, Core_Visuals_Context* context, Core_Assets_Material* material_asset) {
  ...
  dx_rgb_n8_to_rgba_f32(&CORE_ASSETS_COLORRGBN8(material_asset->ambientColor->object)->value, 1.f, &SELF->ambient_color);
  SELF->ambient_texture = NULL;
