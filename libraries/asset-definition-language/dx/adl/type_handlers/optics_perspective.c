...
static dx_result _initialize_expected_keys(dx_adl_type_handlers_optics_perspective* SELF);

static dx_result _uninitialize_expected_keys(dx_adl_type_handlers_optics_perspective* SELF);

static dx_result _check_keys(dx_adl_type_handlers_optics_perspective* SELF, dx_ddl_node* node);

static dx_result _parse_optics_perspective(dx_asset_optics_perspective** RETURN, dx_ddl_node* node, dx_adl_context* context);

static dx_result _parse(dx_object**, dx_adl_type_handlers_optics_perspective*, dx_ddl_node* node, dx_adl_context* context);

static dx_result _resolve(dx_adl_type_handlers_optics_perspective* SELF, dx_adl_symbol* symbol, dx_adl_context* context);
...
static dx_result _parse_optics_perspective(dx_asset_optics_perspective** RETURN, dx_ddl_node* node, dx_adl_context* context) {
  ...
  if (!dx_string_is_equal_to(received_type, NAME(optics_perspective_type))) {
  ...
  // aspectRatio?
  {
    ...
    if (!dx_adl_semantical_read_f32(&value, node, key)) {
      if (!optics_asset->aspect_ratio) {
        if (dx_memory_allocate(&optics_asset->aspect_ratio, sizeof(dx_f32))) {
          DX_UNREFERENCE(optics_asset);
          optics_asset = NULL;
          return DX_FAILURE;
        }
      ...
      *optics_asset->aspect_ratio = value;
      ...
      if (optics_asset->aspect_ratio) {
        dx_memory_deallocate(optics_asset->aspect_ratio);
...
static dx_result _parse(dx_object** RETURN, dx_adl_type_handlers_optics_perspective* SELF, dx_ddl_node* node, dx_adl_context* context) {
  if (!node) {
...
dx_result dx_adl_type_handlers_optics_perspective_construct(dx_adl_type_handlers_optics_perspective* SELF) {
  dx_rti_type* TYPE = dx_adl_type_handlers_optics_perspective_get_type();
  if (!TYPE) {
    return DX_FAILURE;
  }
...
static void dx_adl_type_handlers_optics_perspective_destruct(dx_adl_type_handlers_optics_perspective* SELF) {
  _initialize_expected_keys(SELF);
}

static void dx_adl_type_handlers_optics_perspective_dispatch_construct(dx_adl_type_handlers_optics_perspective_dispatch* SELF) {
