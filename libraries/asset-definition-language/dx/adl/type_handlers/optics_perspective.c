...
static Core_Result _parse_optics_perspective(dx_asset_optics_perspective** RETURN, dx_ddl_node* node, dx_adl_context* context) {
  ...
  {
    Core_Real32 value;
    Core_String* key = NAME(field_of_view_y_key);
...
static Core_Result _parse(Core_Object** RETURN, dx_adl_type_handlers_optics_perspective* SELF, dx_ddl_node* node, dx_adl_context* context) {
  if (!node) {
...
Core_Result dx_adl_type_handlers_optics_perspective_construct(dx_adl_type_handlers_optics_perspective* SELF) {
  DX_CONSTRUCT_PREFIX(dx_adl_type_handlers_optics_perspective);
