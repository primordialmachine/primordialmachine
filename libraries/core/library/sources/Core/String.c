...
Core_Result Core_String_construct(Core_String* SELF, char const* bytes, Core_Size numberOfBytes) {
  Core_BeginConstructor(Core_String);
  ...
  Core_Boolean t;
  if (_utf8_is_utf8_byte_sequence(&t, bytes, numberOfBytes, NULL)) {
    return Core_Failure;
  }
  if (!t) {
    Core_setError(Core_Error_ArgumentInvalid);
  ...
  Core_Memory_copy(SELF->bytes, bytes, numberOfBytes);
  SELF->numberOfBytes = numberOfBytes;
  Core_EndConstructor(Core_String);
}
...
Core_Result Core_String_constructFromArray(Core_String* SELF, Core_InlineArrayListN8* array) {
  ...
  Core_Boolean t;
  if (_utf8_is_utf8_byte_sequence(&t, SELF->bytes, numberOfBytes, NULL)) {
    Core_Memory_deallocate(SELF->bytes);
    SELF->bytes = NULL;
    return Core_Failure;
  }
...
Core_Result Core_String_constructFromSubArray(Core_String* SELF, Core_InlineArrayListN8* array, Core_Size start, Core_Size length) {
  ...
  Core_Boolean t;
  if (!_utf8_is_utf8_byte_sequence(&t, SELF->bytes, length, NULL)) {
    Core_Memory_deallocate(SELF->bytes);
    SELF->bytes = NULL;
    return Core_Failure;
  }
...
Core_Result Core_String_containsSymbol(Core_Boolean* RETURN, Core_String const* SELF, uint32_t symbol) {
  y_utf8_it it = { .current = (uint8_t*)SELF->bytes,
                   .end = (uint8_t*)SELF->bytes + SELF->numberOfBytes,
                   .error = false };
  uint32_t p;
  Core_Size n;
...
Core_Result Core_String_isEqualTo(Core_Boolean* RETURN, Core_String const* SELF, Core_String const* other) {
  ...
  if (SELF->numberOfBytes != other->numberOfBytes) {
    *RETURN = false;
    ...
    Core_Memory_compare(&temporary, SELF->bytes, other->bytes, SELF->numberOfBytes); // must succeed
    *RETURN = !temporary;
  }
  return Core_Success;
}
...
Core_Result Core_String_getHashValue(Core_Size* RETURN, Core_String const* SELF) {
  ...
  return Core_hashBytes(RETURN, SELF->bytes, SELF->numberOfBytes);
}
...
static void dx_string_iterator_impl_constructDispatch(dx_string_iterator_impl_Dispatch* SELF) {
  CORE_STRINGITERATOR_DISPATCH(SELF)->hasValue = (Core_Result(*)(Core_Boolean*, Core_StringIterator*))&dx_string_iterator_impl_has_value;
  CORE_STRINGITERATOR_DISPATCH(SELF)->getValue = (Core_Result(*)(uint32_t*, Core_StringIterator*))&dx_string_iterator_impl_get_value;
  CORE_STRINGITERATOR_DISPATCH(SELF)->next = (Core_Result(*)(Core_StringIterator*))&dx_string_iterator_impl_next;
  CORE_STRINGITERATOR_DISPATCH(SELF)->next_n = (Core_Result(*)(Core_StringIterator*,Core_Size))&dx_string_iterator_impl_next_n;
}

Core_Result dx_string_iterator_impl_construct(dx_string_iterator_impl* SELF, Core_String* string) {
  Core_BeginConstructor(dx_string_iterator_impl);
  ...
  Core_EndConstructor(dx_string_iterator_impl);
}
